...
import 'package:pln_up3_ijendev/Routes/routes.dart';
...
class _SplashScreenState extends State<SplashScreen> {
  ...
  void initState() {
    ...
    // Slideshow: ganti gambar setelah 2 detik
  ...
  Future<void> _decideAndNavigate() async {
    ...
    // Jika sudah login & belum kadaluarsa -> langsung ke daftar alat
    // Jika tidak -> ke halaman login
    final targetRoute = (loggedIn && notExpired)
        ? '/daftaralatpage'
        : Routes.loginPage;

    // Opsional: jika kadaluarsa, bersihkan flag
    if (loggedIn && !notExpired) {
      await prefs.setBool(_kIsLoggedIn, false);
    }

    // Tahan total 4 detik dari splash
